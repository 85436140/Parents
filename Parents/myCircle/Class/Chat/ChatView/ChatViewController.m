...
//    [ChatRecordService deleteTable];
}

- (void)setupBarButtonItem
{
    UIButton *backBtn = [PCBaseVC initWithBtn:@"我的孩子" andBtnFrame:NAVIGATION_RECT_MAX];
    ...
    UIBarButtonItem *leftBBItem = [[UIBarButtonItem alloc] initWithCustomView:backBtn];
    ...
    if (_isChatGroup) {
        ...
        [detailButton setImageEdgeInsets:UIEdgeInsetsMake(0, 40, 0, 0)];
...
{
    NSArray *msgArr = [ChatRecordService queryChatRecord];
    NSString *groupType = [[PC_Globle shareUserDefaults] valueForKey:@"groupType"];
    for (MessageModel *msg in msgArr) {
        if (_isChatGroup && msg.isChatGroup && [msg.groupType isEqual:groupType]) {
            [self.dataSource addObject:msg];
        }
        if (!_isChatGroup && !msg.isChatGroup) {
            [self.dataSource addObject:msg];
        }
    }
...
//    dispatch_async(_messageQueue, ^{
        ...
        [ChatRecordService cacheChatRecored:messages];
