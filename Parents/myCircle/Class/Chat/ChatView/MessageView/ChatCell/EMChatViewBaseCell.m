...
- (id)initWithMessageModel:(MessageModel *)model reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _headImageView = [[UIImageView alloc] initWithFrame:CGRectMake(HEAD_PADDING, CELLPADDING+20, HEAD_SIZE, HEAD_SIZE)];
        ...
        _timeLable = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2-50, 0, 100, 30)];
...
- (void)setMessageModel:(MessageModel *)messageModel
{
    ...
    _timeLable.text = messageModel.lastSendTime;
    _nameLabel.hidden = !messageModel.isChatGroup;
    NSString *sex_p = [[PC_Globle shareUserDefaults] valueForKey:@"sex_p"];
    UIImage *placeholderImage = [UIImage imageNamed:@"mum"];
    if ([sex_p isEqual:@"1"]) {
        placeholderImage = [UIImage imageNamed:@"dad"];
    }
